...
cross-file** (an enum compared as a bare string in one module, a config key with no consumer, a
test path that diverges from the live path), which a diff-only reviewer structurally cannot see.

## What this repo is

<2–4 sentences: stack, what it does, and — critically — **what the consequences of a defect
actually are.** Reviewers calibrate severity off this paragraph. Overstating it ("live money!"
when there is no live environment) inflates every judgment call toward the most conservative
reading and buries real findings under theoretical ones. Understating it does the reverse. Get it
accurate and keep it accurate.>
...
> a real incident.** An invariant list assembled from best-practice articles rather than from your
> own incident history will be long, generic, and ignored.
...
5. **ENUM / STRING DRIFT.** Comparisons against string literals of enum values are silent no-ops
   waiting to happen — require `Enum.X.value` or normalization at the boundary, plus a canary test
   pinning the enum set.
6. **MIGRATIONS.** Exactly one head before and after; every migration has a working `downgrade()`;
   backfills are **additive only** and never mutate operational switches.
...
8. **TEST vs. LIVE DIVERGENCE.** Any wall-clock, feed-age, or alerting logic must state its
   behavior in each mode explicitly. Flag any look-ahead or future-data leak.
9. **ALERTS.** Notifications fire on state **transitions** only, never per-cycle. Anything meaning
   "a protection is not active right now" is an INCIDENT, not a footnote.
...
- **Review the code that is there, not the code you would have written.** An unfamiliar structure
  or an approach you would not have chosen is **not a finding** — say nothing unless you can name
  the input that makes it fail or the invariant it breaks. *A reviewer that taxes novelty trains
  the author out of it.* If a design worries you but you cannot produce a failing case, that is at
  most a `[P3]` framed as a question.
...
- **A well-evidenced author rebuttal closes a thread.** You are advisory. If the author replies
...
clean.** Without it, a clean PR is indistinguishable from one nothing reviewed, which forces a
second reviewer just to confirm coverage. **Never manufacture findings to fill it.**
...
Then one line naming what you actually traced (e.g. "checked: deploy path, migration head count,
enum/string parity, fail directions, test-vs-live"). If not clean, list each finding as
...
Apply the same rigor to **claims**: flag a consequential claim lacking the stamp grammar
(`STATUS: CANDIDATE|VALIDATED|REJECTED · evidence: <path>`), a mislabeled result, a dead
cross-reference, a citation pointing at a line that no longer exists, or a contradiction with
